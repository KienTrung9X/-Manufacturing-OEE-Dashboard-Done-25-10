...
const ProductionLogTable: React.FC<ProductionLogTableProps> = ({ data, onMachineSelect, oeeThreshold, allDefectTypes, allDefectRecords, onNavigateToLog }) => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-700">
            <tr>
              <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-white sm:pl-6"><button onClick={() => requestSort('MACHINE_ID')} className={headerButtonClass}>{t('machine')} {getSortIcon('MACHINE_ID')}</button></th>
              ...
              >
                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 dark:text-white sm:pl-6">
                  <button
                    ...
                    className="group flex items-center gap-3 hover:text-cyan-500 dark:hover:text-cyan-400 focus:outline-none"
                  >
                    <span
                    ...
                    >
                      <span
                        className={`h-2 w-2 rounded-full ${
                          log.STATUS === 'active' ? 'bg-green-500' : 'bg-gray-400'
                        }`}
                      ></span>
                      {log.STATUS === 'active' ? t('active') : t('inactive')}
                    </span>
                    <span className="group-hover:underline">{log.MACHINE_ID}</span>
                  </button>
                </td>
