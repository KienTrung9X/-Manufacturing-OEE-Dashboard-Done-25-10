        ...
        <main className="p-6 overflow-y-auto bg-gray-50 dark:bg-gray-900">
          {!machineInfo || !productionRecord ? (
            ...
            <div className="space-y-6">
                        ...
                        {maintenanceHistory.map(order => (
                            <li key={order.id} className="py-3">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="font-semibold text-gray-800 dark:text-gray-200">{t('order')} #{order.id}: <span className="font-normal text-gray-600 dark:text-gray-400">{order.task_description}</span></p>
                                        <p className="text-xs text-gray-500">{t('completed')}: {order.actual_end_date ? new Date(order.actual_end_date).toLocaleDateString() : 'N/A'} {t('by')} {order.assigned_to_name}</p>
                                    </div>
                                    <span className="font-bold text-xs uppercase tracking-wider text-cyan-400">{t(order.type as any)}</span>
                                </div>
                                {order.parts_used && order.parts_used.length > 0 && (
                                    <div className="mt-2 pl-4 border-l-2 border-gray-200 dark:border-gray-600">
                                        <h5 className="text-xs font-semibold text-gray-500 dark:text-gray-400">{t('partsUsedTitle')}</h5>
                                        <ul className="text-xs list-disc pl-5 mt-1 space-y-1">
                                            {order.parts_used.map(part => (
                                                <li key={part.part_id} className="text-gray-600 dark:text-gray-300">
                                                    {part.part_name} ({part.part_code}) - <span className="font-semibold">{t('qty')}: {part.qty_used}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
              ...
              <Card title={t('sparePartReplacementHistory')}>
                {(() => {
                    const historyWithParts = maintenanceHistory.filter(order => order.parts_used && order.parts_used.length > 0);
                    if (historyWithParts.length === 0) {
                        return <p className="text-gray-400 text-sm">{t('noSparePartHistory')}</p>;
                    }
                    return (
                        <ul className="divide-y divide-gray-200 dark:divide-gray-700 max-h-48 overflow-y-auto text-sm">
                            {historyWithParts.flatMap(order => 
                                order.parts_used.map(part => (
                                    <li key={`${order.id}-${part.part_id}`} className="py-2 grid grid-cols-3 gap-4 items-center">
                                        <div>
                                            <p className="font-semibold text-gray-800 dark:text-gray-200">{part.part_name}</p>
                                            <p className="text-xs font-mono text-gray-500">{part.part_code}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-gray-500">{t('qty')}: <span className="font-bold text-white">{part.qty_used}</span></p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-xs text-gray-500">{t('date')}: {order.actual_end_date ? new Date(order.actual_end_date).toLocaleDateString() : 'N/A'}</p>
                                            <p className="text-xs text-gray-500">{t('order')} #{order.id}</p>
                                        </div>
                                    </li>
                                ))
                            )}
                    ...
                    <ul className="divide-y divide-gray-200 dark:divide-gray-700 max-h-48 overflow-y-auto text-sm">
                        {plannedMaintenance.map(order => (
                            <li key={order.id} className="py-2">
                                <p className="font-semibold text-gray-800 dark:text-gray-200">{t('task')}: <span className="font-normal text-gray-600 dark:text-gray-400">{order.task_description}</span></p>
                                <p className="text-xs text-gray-500">{t('scheduledFor')}: {new Date(order.plan_date).toLocaleDateString()}</p>
                            </li>
                        ))}
                    </ul>
