...
const MaintenanceOrderModal: React.FC<MaintenanceOrderModalProps> = ({ isOpen, onClose, onSubmit, allMachines, allUsers, allDefectCauses, openDefects, currentDate, allSpareParts, defaults }) => {
    ...
    const getInitialState = () => ({
        ...
        type: defaults?.type || 'PM' as 'PM' | 'IM',
        priority: 'Medium' as 'Low' | 'Medium' | 'High',
        created_by_id: allUsers.find(u => u.role === 'Maintenance')?.id || 0,
        task_description: defaults?.task_description || (defaults?.type === 'PM' ? 'Scheduled Preventive Maintenance' : ''),
        plan_date: defaults?.plan_date || currentDate,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setError('');
        if (!formData.machine_id || !formData.created_by_id) { 
            setError('Please select a machine and creator.');
            return;
        }
    ...
    const maintenanceUsers = allUsers.filter(u => u.role === 'Maintenance' || u.role === 'Admin' || u.role === 'Supervisor');
                ...
                <form id="maintenance-order-form" onSubmit={handleSubmit}>
                    <main className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <FormField label={t('orderType')} id="type" required><select name="type" value={formData.type} onChange={handleChange} className={formInputClass}><option value="PM">{t('pm')}</option><option value="IM">{t('IM')}</option></select></FormField>
                        ...
                        <FormField label={t('machine')} id="machine_id" required><select name="machine_id" value={formData.machine_id} onChange={handleChange} className={formInputClass}>{allMachines.map(m => <option key={m.id} value={m.id}>{m.MACHINE_ID} - {m.MACHINE_NAME} (Line {m.LINE_ID})</option>)}</select></FormField>
                        
                        <FormField label={t('taskDescription')} id="task_description" required><textarea name="task_description" value={formData.task_description} onChange={handleChange} rows={4} className={formInputClass} placeholder={t('describeIssue')}></textarea></FormField>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           <FormField label={t('planDate')} id="plan_date" required><input type="date" name="plan_date" value={formData.plan_date} onChange={handleChange} className={formInputClass} required /></FormField>
                           <FormField label={t('createdBy')} id="created_by_id" required><select name="created_by_id" value={formData.created_by_id} onChange={handleChange} className={formInputClass}>{maintenanceUsers.map(u => <option key={u.id} value={u.id}>{u.full_name} ({u.role})</option>)}</select></FormField>
