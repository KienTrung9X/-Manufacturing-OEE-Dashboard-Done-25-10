...
const ErrorReportModal: React.FC<ErrorReportModalProps> = ({ isOpen, onClose, onSubmit, onUpdate, reportToUpdate, masterData, openMaintenanceOrders, defaults }) => {
    ...
    const getInitialCreateState = () => ({
        machine_id: defaults?.machine_id || masterData?.machines[0]?.id || 0,
        shift_id: defaults?.shift_id || masterData?.shifts[0]?.id || 0,
        defect_type: defaults?.defect_type || masterData?.defectTypes[0]?.name || '',
        defect_description: defaults?.defect_description || '',
        severity: defaults?.severity || 'Medium' as 'Low' | 'Medium' | 'High',
    });
