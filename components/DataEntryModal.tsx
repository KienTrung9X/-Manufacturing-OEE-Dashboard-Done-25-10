...
import { NewDefectData, MachineInfo, Shift, DefectType, DefectCause, EnrichedMaintenanceOrder, EnrichedDefectRecord } from '../types';
...
const DataEntryModal: React.FC<DataEntryModalProps> = ({ isOpen, onClose, onSubmit, allMachines, allShifts, allDefectTypes, allDefectCauses, openMaintenanceOrders, currentDate, onOpenLinkedErrorReport }) => {
    const { t } = useTranslation();
    ...
    const handleOrderLinkChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (orderId) {
            ...
            if (selectedOrder) {
                setFormData(prev => ({ ...prev, machine_id: selectedOrder.machine_id, note: selectedOrder.task_description }));
            }
        } else {
            setFormData(prev => ({ ...prev, machine_id: allMachines[0]?.id || 0, note: '' }));
        }
