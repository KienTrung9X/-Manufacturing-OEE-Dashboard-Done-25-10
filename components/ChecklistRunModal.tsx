...
const ChecklistRunModal: React.FC<ChecklistRunModalProps> = ({ isOpen, onClose, onSubmit, checklistInstance }) => {
    ...
    const handleSubmit = () => {
        // Transform the local state back into the full ChecklistInstanceItem array
        const updatedItems: ChecklistInstanceItem[] = checklistInstance.items.map(originalItem => {
            const currentState = itemsState[originalItem.id];
            return {
                ...originalItem,
                status: currentState.status,
                actual_value: currentState.actual_value,
                notes: currentState.notes,
            };
        });
        onSubmit(checklistInstance.id, updatedItems);
    };
